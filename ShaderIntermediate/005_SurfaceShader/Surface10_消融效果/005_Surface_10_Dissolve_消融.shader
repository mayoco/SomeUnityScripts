Shader "Custom/005_Surface_10_Dissolve"
{
    Properties
    {
		...
		_BurnIntensity("BurnIntensity",Range(0,5))=1
    }
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
			...
			float cutout = tex2D(_DissolveTex, IN.uv_DissolveTex).r;
			clip(cutout - _Threshold);
			float temp = saturate((cutout - _Threshold)/_EdgeLength);
			fixed4 edgeColor = tex2D(_BurnTex,float2(temp,temp));
			fixed4 finalColor = _BurnIntensity * lerp(edgeColor,fixed4(0,0,0,0),temp);
